$(document).ready(function() {

    $("#add_brand_form").submit(function(event) {

        event.preventDefault();

        var trigger_btn = $(this).find("button[type='submit']");
        var server_response = $(this).find("span[class='server_response']");

        var spinner = "<span class='fas fa-spinner fa-spin'></span> Adding Brand";

        trigger_btn.html(spinner);

        var form_data = $(this).serialize();

        $.ajax({
            url: admin_url() + "models/add_brand.php",
            type: "POST",
            data: form_data,
            success: function(response){

                response = $.trim(response);

                if(response === "Brand Added Successfully"){

                    window.location = admin_url() + "all_brands";

                }else{

                    server_response.html(response);
                    trigger_btn.html("Add Brand");

                }

            }
        });

    });

});
